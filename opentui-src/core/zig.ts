...
const localNativeCandidates = [
  ...
  join(repoRoot, "opentui-src", "core", "zig", "zig-out", "lib", "libopentui.dylib"),
  join(repoRoot, "opentui-src", "core", "zig", "lib", `${zigArch}-${zigOs}`, "libopentui.dylib"),
]

let targetLibPath = localNativeCandidates.find((candidate) => existsSync(candidate))

if (!targetLibPath) {
  throw new Error(`libopentui.dylib not found in any candidate path. Searched:\n${localNativeCandidates.join("\n")}`)
}
