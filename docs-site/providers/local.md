...
LongerAgent supports three local inference servers: Ollama, oMLX, and LM Studio. These run models on your own hardware with no API key required.

## How Local Providers Work

All local providers use the OpenAI-compatible Chat Completions API. During `longeragent init`, the wizard queries the server's `/v1/models` endpoint to discover available models automatically.
...
3. Run `longeragent init` and select **Ollama (Local)**.
...
2. Run `longeragent init` and select **oMLX (Local)**.
...
3. Run `longeragent init` and select **LM Studio (Local)**.
...
- Make sure the server is running **before** you run `longeragent init`. The wizard needs to query it for available models.
- If you change models in your local server, re-run `longeragent init` to update LongerAgent's configuration.
- Local models generally have lower context windows than cloud models. LongerAgent's context management (summarize, compact) becomes especially important for keeping sessions productive.
