...
`classifyToolAsync(toolName, args)` —— 异步版，bash 必须走这条（要 tree-sitter 解析）。对 `cp`/`mv` 命令，分类后还会检查目标路径的文件系统状态：目标是已有目录时升级为 `write_potent`（触发审批）。compound 命令中只检查包含 cp/mv 的那个 segment。
...
Bash 命令分类：
- `mkdir` → `write_reversible`（总是可逆，rewind 可自动 rmdir）
- `cp`/`mv`（单 source，目标非已有目录）→ `write_reversible`（有 mutation 跟踪）
- `cp`/`mv`（目标是已有目录）→ `write_potent`（需审批）
- `cp`/`mv`（多 source）→ 不跟踪 mutation（标 untracked）
- `touch`/`ln` → `write_potent`（无 rewind 支持）

## Bash Mutation Tracking

`BashMutation`（`tools/basic.ts`）记录 `mkdir`/`cp`/`mv` 的可逆操作。bash executor 在执行前 snapshot 文件系统状态，成功后记录 `BashMutationEntry` 到 tool_result metadata。

rewind 时按时间逆序统一处理 file mutations 和 bash mutations（不分阶段），确保同路径上的操作按正确顺序撤销。compound bash 的多个 entry 按 per-entry 粒度标记 reverted（`bashMutationRevertedIndices`）。

`rewindBoth` 安全阀：当存在未回退的 conflict 时，不截断 conversation log，防止丢失后续重试所需的日志。
